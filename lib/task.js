...
Task.prototype._createPlan = function createPlan(data) {
  ...
  for(var i=0; i<task_args.length; i++){
    ...
    if(auto_task_type) {
      ...
      // if(auto_task_type != 'context') plan.auto_task.push(task_arg);
  ...
  // if(flow.is_subflow) plan.auto_task.push(flow.context_name);  //ensure that if there are any context properties???????????  what about context properties?
...
//   var self = this;
//   var root_flow = self;
//   while(root_flow.parent) root_flow = root_flow.parent;
//   var data = root_flow.data;
//   requirements = requirements || {data: {}, tasks: {}};
//   var plan = self._createPlan();
//   for(var i=0; i<(plan.auto_task.length-1); i++) {
//     var prereq = plan.auto_task[i];
//   }
// }
