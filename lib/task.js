...
Task.prototype._createPlan = function createPlan() {
  ...
  for(var i=0; i<task_args.length; i++){
    ...
    var auto_task_type = flow.tasks.hasOwnProperty(task_arg) && 'task' || 
      flow.flows.hasOwnProperty(task_arg) && 'task' || 
      flow.data_keys.hasOwnProperty(task_arg) && 'data' || 
      flow.context_keys.hasOwnProperty(task_arg) && 'context';
    if(task_arg == self.name) auto_task_type = null;
    if(auto_task_type) {
      if(auto_task_type == 'task') task._task_requirements.push(task_arg);
      else if(auto_task_type == 'data' || auto_task_type == 'context') task._data_requirements.push(task_arg);
  ...
  plan.auto_task.push(function(callback, results){
    ...
    if(typeof plan.fn != 'function') {
      ...
      plan.fn = receiver[fnName];
      if(!plan.fn) throw new FlowTaskError(self.name, "Unknown symbol '" + fnName + "' must be either the name of a task, the name of data, or the name of a function on '" + receiverName + "'");
      if(typeof plan.fn != 'function') throw new FlowTaskError(self.name, "Not a function: " + (receiverName.length ? (receiverName.join('.') + ".") : '') + fnName);
    }

    try {
      var return_value = plan.fn.apply(receiver, fnArgs);
