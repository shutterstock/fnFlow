...
var flowExec = function flowExec(data, tasks, contextName, callback) {
  ...
  if (data instanceof Array) async.map(data, exec, callback);
  else exec(data, callback);
}
...
var flowGo = function flowGo(data, tasks, context, callback) {
  ...
  if(tasks) us.extend(newTasks, us.object(Object.keys(tasks), us.map(Object.keys(tasks), function(key){ 
    var task = Array.isArray(tasks[key]) ? new AsyncTask(tasks[key]) : tasks[key];
    var taskPlan = interpretTask(task, key, tasks, data, context);
...
var interpretTask = function interpretTask(taskArgs, taskName, tasks, data, context) {
  ...
  return { fn: fn, args: args, autoTask: autoTask, isSync: (taskArgs instanceof SyncTask) };
}
...
var Task = function(task_array){
  this.task_array = task_array;
}

var AsyncTask = function(task_array){
...
var SyncTask = function(task_array){
