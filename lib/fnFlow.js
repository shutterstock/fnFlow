    ...
    for(var i in task.task_array) {
      ...
      if(typeof taskArg == 'function') {
        ...
        if(auto_task_type) {
          ...
          else if(auto_task_type == 'data') task.data_requirements.push(taskArg);
  ...
  if(typeof fn == 'object' && fn.tasks) {  //subflows
    ...
    autoTask.push(function(callback, results){
      ...
      if(typeof fn != 'function') {
        ...
        if(!receiver) return callback(new FlowTaskArgumentNullError(taskName, receiverName, fnName));  //do assertExists
        fn = receiver[fnName];
