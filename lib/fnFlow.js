  ...
  if(taskArgs instanceof SubFlow) {
    ...
    for(var i in taskArgs) {
      ...
      if(typeof taskArg == 'function') {
        ...
        /*
        if there's one part,
          if it's a task or data
            if fn push to args
            push to task
          else
            if fn error
            set fn to receiver: previous taskArg, name: taskArg
            if !fn.receiverName error
        else
          if the first part is a task or data
            if fn push array of parts to args
            else if !fn set fn to receiver: array of all but last part, name: last part
            push first part as task
          else 
            error

        taskArg = first part
        if taskArg is a task or data
          if fn 
            if taskArg is a string push taskArg to args
            else push 
        */
      } else throw new FlowTaskError(taskName, "Unknown symbol at index '" + i + "' must be either the name of a task, the name of data, or be the name of a function on the result of a task or data");
    }
