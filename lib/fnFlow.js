...
Flow.prototype._createPlan = function _createPlan(data) {
  ...
  var planTasks = function(o){
    ...
    keys.forEach(function(item_name){
      ...
      if(item._createPlan) {
        var item_plan = item._createPlan(data);
        var item_prereqs = item_plan.auto_task.slice(0, item_plan.length - 1);
        for(var i=0; i<item_prereqs.length; i++) {
          var item_prereq = item_prereqs[i];
          var is_context = !(plan.prereqs[item_prereq] = data && data.hasOwnProperty(item_prereq) || self.tasks.hasOwnProperty(item_prereq) || self.flows.hasOwnProperty(item_prereq));
          if(is_context && item_prereq != context_name) {
            plan.auto[item_prereq] = plan.auto[item_prereq] || function(cb, results){ 
  ...
  // if(self.context_name) planTasks(self.context_name);  //watch for thread safety here...hey, maybe i only need the names for prereqs that don't match?
  if(self.context_name) plan.prereqs[self.context_name.split('.')[0]] = true;
  ...
  plan.auto_task.push(function(cb, results){
    ...
    if(context === null || context === undefined) {
      throw new FlowTaskError(self.name, "Result of '" + self.context_name + "' returned no data. Could not start SubFlow.");
